...
server.registerTool('family_ask', { description: 'Ask in Russian about Hebrew chat history; translates search terms and answers with original sources. Does not send a notification.', inputSchema: { question: z.string().min(1).max(2000) }, annotations }, async ({ question }) => result('/ask', { question }));
server.registerTool('family_summary', { description: 'Generate a Russian digest for a period such as 24h or 7d, optionally filtered by group name. Does not send to Telegram. May take several minutes for busy periods.', inputSchema: { period: z.string().regex(/^\d+[hd]$/).default('24h'), group: z.string().optional() }, annotations }, async ({ period, group }) => result('/summary', { period, group }));
